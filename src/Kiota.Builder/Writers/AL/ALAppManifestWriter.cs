...
public class ALAppManifestWriter : BaseElementWriter<CodeFunction, ALConventionService>
{
    ...
    public override void WriteCodeElement(CodeFunction codeElement, LanguageWriter writer)
    {
        ...
        if (codeElement.Name.Equals("Readme", StringComparison.OrdinalIgnoreCase))
        {
            WriteReadme(codeElement, writer);
            return;
        }
...
#pragma warning restore IL2026
    ...
    private static void WriteReadme(CodeFunction codeElement, LanguageWriter writer)
    {
        var config = ExtractConfig(codeElement);
        ...
        var idRangeStart = GetConfigValue(config, "IDRangeStart", "50000");
        var idRangeEnd = GetConfigValue(config, "IDRangeEnd", "99999");
        var companionNs = GetConfigValue(config, "CompanionNamespace", "Fps.Kiota");
        var generateInterfaces = GetConfigValue(config, "GenerateInterfaces", "False");

        writer.WriteLine("# Auto-Generated AL Client", includeIndent: false);
        writer.WriteLine(string.Empty, includeIndent: false);
        writer.WriteLine("This code was auto-generated by [Kiota](https://aka.ms/kiota).", includeIndent: false);
        writer.WriteLine(string.Empty, includeIndent: false);
        writer.WriteLine("## Generation Parameters", includeIndent: false);
        writer.WriteLine(string.Empty, includeIndent: false);
        writer.WriteLine("| Parameter | Value |", includeIndent: false);
        writer.WriteLine("|---|---|", includeIndent: false);
        writer.WriteLine($"| OpenAPI spec | `{openApiPath}` |", includeIndent: false);
        writer.WriteLine($"| Language | `{language}` |", includeIndent: false);
        writer.WriteLine($"| Client class name | `{clientClassName}` |", includeIndent: false);
        writer.WriteLine($"| Client namespace | `{clientNamespace}` |", includeIndent: false);
        writer.WriteLine($"| Output path | `{outputPath}` |", includeIndent: false);
        writer.WriteLine(string.Empty, includeIndent: false);
        writer.WriteLine("## AL Configuration", includeIndent: false);
        writer.WriteLine(string.Empty, includeIndent: false);
        writer.WriteLine("| Parameter | Value |", includeIndent: false);
        writer.WriteLine("|---|---|", includeIndent: false);
        if (!string.IsNullOrEmpty(objectPrefix))
            writer.WriteLine($"| Object prefix | `{objectPrefix}` |", includeIndent: false);
        if (!string.IsNullOrEmpty(objectSuffix))
            writer.WriteLine($"| Object suffix | `{objectSuffix}` |", includeIndent: false);
        writer.WriteLine($"| Object ID range | `{idRangeStart}` - `{idRangeEnd}` |", includeIndent: false);
        writer.WriteLine($"| Companion namespace | `{companionNs}` |", includeIndent: false);
        writer.WriteLine($"| Generate interfaces | `{generateInterfaces}` |", includeIndent: false);

        // Include / Exclude patterns
        if (config.TryGetValue("IncludePatterns", out var includePatterns) && !string.IsNullOrEmpty(includePatterns))
        {
            writer.WriteLine(string.Empty, includeIndent: false);
            writer.WriteLine("## Include Patterns", includeIndent: false);
            writer.WriteLine(string.Empty, includeIndent: false);
            foreach (var pattern in includePatterns.Split(','))
                writer.WriteLine($"- `{pattern.Trim()}`", includeIndent: false);
        }

        if (config.TryGetValue("ExcludePatterns", out var excludePatterns) && !string.IsNullOrEmpty(excludePatterns))
        {
            writer.WriteLine(string.Empty, includeIndent: false);
            writer.WriteLine("## Exclude Patterns", includeIndent: false);
            writer.WriteLine(string.Empty, includeIndent: false);
            foreach (var pattern in excludePatterns.Split(','))
                writer.WriteLine($"- `{pattern.Trim()}`", includeIndent: false);
        }

        // Reconstruct the kiota generate command
        writer.WriteLine(string.Empty, includeIndent: false);
        writer.WriteLine("## Regenerate", includeIndent: false);
        writer.WriteLine(string.Empty, includeIndent: false);
        writer.WriteLine("To regenerate this client, run:", includeIndent: false);
        writer.WriteLine(string.Empty, includeIndent: false);
        writer.WriteLine("```shell", includeIndent: false);

        var cmd = $"kiota generate --openapi \"{openApiPath}\" --language {language} --class-name {clientClassName} --namespace-name {clientNamespace} --output \"{outputPath}\"";

        if (config.TryGetValue("IncludePatterns", out var incPatterns) && !string.IsNullOrEmpty(incPatterns))
        {
            foreach (var pattern in incPatterns.Split(','))
                cmd += $" --include-path \"{pattern.Trim()}\"";
        }

        if (config.TryGetValue("ExcludePatterns", out var excPatterns) && !string.IsNullOrEmpty(excPatterns))
        {
            foreach (var pattern in excPatterns.Split(','))
                cmd += $" --exclude-path \"{pattern.Trim()}\"";
        }

        writer.WriteLine(cmd, includeIndent: false);
        writer.WriteLine("```", includeIndent: false);

        // al-config.json contents
        writer.WriteLine(string.Empty, includeIndent: false);
        writer.WriteLine("## AL Configuration File (`al-config.json`)", includeIndent: false);
        writer.WriteLine(string.Empty, includeIndent: false);
        writer.WriteLine("```json", includeIndent: false);
        writer.WriteLine("{", includeIndent: false);
        writer.WriteLine($"  \"objectPrefix\": \"{objectPrefix}\",", includeIndent: false);
        writer.WriteLine($"  \"objectSuffix\": \"{objectSuffix}\",", includeIndent: false);
        writer.WriteLine($"  \"objectIdRangeStart\": {idRangeStart},", includeIndent: false);
        writer.WriteLine($"  \"objectIdRangeEnd\": {idRangeEnd},", includeIndent: false);
        writer.WriteLine($"  \"companionNamespace\": \"{companionNs}\",", includeIndent: false);
        writer.WriteLine($"  \"generateInterfaces\": {generateInterfaces.ToLowerInvariant()}", includeIndent: false);
        writer.WriteLine("}", includeIndent: false);
        writer.WriteLine("```", includeIndent: false);
    }

    private static Dictionary<string, string> ExtractConfig(CodeFunction codeElement)
    {
